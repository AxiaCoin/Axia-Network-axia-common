// Copyright 2020 AXIA Technologies
//
...
fn malloc_size_of_derive(s: synstructure::Structure) -> proc_macro2::TokenStream {
		...
		if ignore {
			...
			Some(quote! {
				for item in #binding.iter() {
					sum += axia_util_mem::MallocSizeOf::size_of(item, ops);
				}
			})
		} else {
			Some(quote! {
				sum += axia_util_mem::MallocSizeOf::size_of(#binding, ops);
			})
		}
	});
	...
	for param in ast.generics.type_params() {
		...
		where_clause
			.predicates
			.push(parse_quote!(#ident: axia_util_mem::MallocSizeOf));
	}

	let tokens = quote! {
		impl #impl_generics axia_util_mem::MallocSizeOf for #name #ty_generics #where_clause {
			#[inline]
